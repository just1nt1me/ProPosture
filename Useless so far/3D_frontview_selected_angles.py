...
with mp.solutions.pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
    while cap.isOpened():
        ...
        try:
            ...
            ### LEFT X-DISTANCE
            left_x_distance = round(l_wrist[0]-l_shoulder[0],4)
            ...
            ### LEFT BUT XZ-DISTANCE
            left_xz_distance = round(math.sqrt((l_wrist[0] - l_shoulder[0])**2 + (l_wrist[2] - l_shoulder[2])**2),4)
            left_xz_distance_text_position = (image.shape[1] - 220, 270)
            cv2.putText(image, str(f'left_x_z_distance: {left_xz_distance}'),
                           left_xz_distance_text_position,
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)
